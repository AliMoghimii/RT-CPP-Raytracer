...
| `raytracer.comp` | `shaders/legacy/` |
| `raytracer.comp.spv` | `shaders/legacy/` (regenerated by `compile_shaders.bat`) |
...
While the refactor is in progress, the existing monolithic shader (`raytracer.comp`) keeps running the entire renderer. It will be progressively replaced by the new pass-based shaders we'll write in Part B onwards. Until the new pipeline fully replaces it, it lives in `shaders/legacy/` as a clear marker of transitional code.

When the refactor completes — primary visibility, probe trace, cascade merge, final gather, and tonemap are all working — you delete the entire `legacy/` folder in one commit titled something like "Remove legacy monolithic shader; new pipeline canonical." Clean ending to the refactor saga.
...
Should show `raytracer.comp` and (after running `compile_shaders.bat` in the next step) `raytracer.comp.spv`. If the `.spv` is missing, that's expected — the next step regenerates it.

### Update the shader path in C++ code

The C++ side loads the compiled SPIR-V via a path. Find the line that reads the shader file (likely in `VulkanCore.cpp` inside `createComputePipeline` or similar) and update the path:

Before:
```cpp
auto computeShaderCode = readFile("shaders/raytracer.comp.spv");
```

After:
```cpp
auto computeShaderCode = readFile("shaders/legacy/raytracer.comp.spv");
```

If the project uses a different path convention (e.g. relative to executable, or with a `$(ProjectDir)`-style prefix), update accordingly. The principle is the same: the path must point to `shaders/legacy/raytracer.comp.spv`.
...
- **Walks every shader subfolder** (`legacy`, `visibility`, `rc`, `shading`, `tonemap`) and compiles every `.comp`/`.vert`/`.frag` it finds. As you add new pass shaders during Phase 1+ they're picked up automatically — no script edits needed.
...
The current `raytracer.comp` is ~970 lines doing everything. Split it into reusable GLSL headers + small per-pass shaders.
...
Verify the current `raytracer.comp` still compiles and produces the same image after switching to `#include` of the new common headers. The pixel output should be byte-identical.
